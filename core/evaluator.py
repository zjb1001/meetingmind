...
class MeetingEvaluator:
    ...
    def __init__(self, config: Optional[EvaluationConfig] = None):
        ...
        Args:
            config: 评价配置，None 则使用默认配置
        """
        app_config = get_config()
        self.config = config or app_config.evaluation

        logger.info(f"初始化评价器，权重配置: 完整性={self.config.completeness_weight:.0%}, "
                   f"准确性={self.config.accuracy_weight:.0%}, "
                   f"结构={self.config.structure_weight:.0%}, "
                   f"行动项={self.config.action_items_weight:.0%}, "
                   f"可读性={self.config.readability_weight:.0%}")
    ...
    def evaluate(self, transcript: str, minutes: str) -> EvaluationResult:
        ...
        if not transcript or not minutes:
            raise ValueError("转录文本和会议纪要不能为空")

        logger.info("开始评价会议纪要...")

        # 各维度评价
        dimensions = {
            'completeness': self._evaluate_completeness(transcript, minutes),
            'accuracy': self._evaluate_accuracy(transcript, minutes),
            'structure': self._evaluate_structure(minutes),
            'action_items': self._evaluate_action_items(transcript, minutes)['score'],
            'readability': self._evaluate_readability(minutes)
        }
        ...
        logger.info(f"评价完成: 总分 {overall_score:.1f}/100")
    ...
    def _calculate_overall_score(self, dimensions: Dict[str, float]) -> float:
        ...
        weights = {
            ...
            'action_items': self.config.action_items_weight,
            'readability': self.config.readability_weight
        }

        return sum(
            score * weights.get(dim, 0.2)
    ...
    def _evaluate_completeness(self, transcript: str, minutes: str) -> float:
        ...
        if transcript_topics:
            ...
            logger.debug(f"议题覆盖度: {coverage:.0%} ({len(minutes_topics)}/{len(transcript_topics)})")
        ...
        if not minutes_attendees:
            score -= 15
            logger.debug("扣分: 缺少参会人员信息")

        # 检查是否有结论/总结
        if '会议总结' not in minutes and '总结' not in minutes:
            score -= 10
            logger.debug("扣分: 缺少会议总结")
    ...
    def _evaluate_accuracy(self, transcript: str, minutes: str) -> float:
        ...
        if minutes_names and minutes_names.issubset(transcript_names):
            score = min(100, score + 5)
        elif minutes_names:
            score -= 10
            logger.debug(f"扣分: 人名不一致 {minutes_names - transcript_names}")
    ...
    def _evaluate_structure(self, minutes: str) -> float:
        ...
        for section in required_sections:
            if section not in minutes:
                score -= 15
                logger.debug(f"扣分: 缺少章节 '{section}'")

        # 检查 Markdown 格式
        if '|' not in minutes:  # 缺少表格
            score -= 10
            logger.debug("扣分: 缺少表格")
    ...
    def _evaluate_action_items(self, transcript: str, minutes: str) -> Dict[str, Any]:
        """评价行动项质量"""
        result: Dict[str, Any] = {
            'score': 100.0,
            'count': 0,
        ...
        if not action_items:
            ...
            logger.debug("行动项: 未找到行动项")
            return result

        for item in action_items:
            # 检查 Who
            if item.get('负责人') or item.get('who') or item.get('owner'):
                result['with_who'] += 1

            # 检查 What
            if item.get('任务描述') or item.get('what') or item.get('task'):
                result['with_what'] += 1

            # 检查 When
            if item.get('截止时间') or item.get('when') or item.get('deadline'):
                result['with_when'] += 1
        ...
        logger.debug(
            f"行动项: {result['count']} 项, "
            f"完整度 {filled_fields}/{total_fields}"
        )
    ...
    def _evaluate_readability(self, minutes: str) -> float:
        ...
        # 检查是否有列表项
        if '-' not in minutes and '•' not in minutes and '*' not in minutes:
            score -= 10

        # 检查冗余词汇
        redundant_words = ['这个', '那个', '然后', '就是']
        for word in redundant_words:
            count = minutes.count(word)
            if count > 5:
                score -= min(10, count * 0.5)
    ...
    def _extract_action_items(self, minutes: str) -> List[Dict[str, str]]:
        """从纪要中提取行动项"""
        items = []
    ...
    def _generate_summary(self, overall_score: float, dimensions: Dict[str, float]) -> str:
        """生成总体评价摘要"""
        if overall_score >= 90:
...
def evaluate_meeting_minutes(
    ...
    if output_file:
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(report)
        logger.info(f"评价报告已保存: {output_file}")
